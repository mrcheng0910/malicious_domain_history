...
from obtaining_rc_ttl import manage_rc_ttl
...
def fetch_mal_domains():
    ...
    sql = 'SELECT domain, malicious_type FROM malicious_domains_index LIMIT 143244, 71622'
    db.query(sql)
...
def main():
    ...
    domain_count = len(mal_domains)
    for check_domain,mal_type in mal_domains:
        print "查询的域名：", domain_count, check_domain, datetime.now().strftime("%Y-%m-%d %H:%M:%S")
