...
class Static(object):
    ...
    value_first_init = False
    log_first_init = False
    # 配置文件路径
    ...
    def static_value_init():
        """静态值初始化"""
        # 只进行一次初始化
        if not Static.value_first_init:
            Static.value_first_init = True
        else:
            return
        # 获取当前绝对路径
        sys.path.append('..')
    ...
    def log_init():
        """对象出初始化"""
        # 只进行一次初始化
        if not Static.log_first_init:
            Static.log_first_init = True
        else:
            return
