...
class WhoisRecord(object):
    ...
    def Update(self, whois_dict, old_flag):
        ...
        if whois_dict['domain_status'].strip() == '29':  # 只更新domain 表
            ...
            self.DB.execute_no_return(SQL_generate.SET_WHOIS_RAW_INFO(whois_dict))

        else:  # 之前已有成功获取的数据
            record_transform = False  # whois 记录归档标志
            table = 'WHOIS_' + str(Static.get_table_num(whois_dict['domain']))
            get_whois_info_SQL = SQL_generate.GET_WHOIS_INFO(whois_dict['domain'],
                                                             'updated_date',
                                                             table)
            origin_update_date = self.DB.execute(get_whois_info_SQL)
            if origin_update_date is None or str(old_flag) == '0':  # 之前不存在的记录 : 单独插入更新whois
                ...
                if record_transform:
                    whowas_table = 'WHOWAS_' + str(Static.get_table_num(whois_dict['domain']))
                    whois_table = 'WHOIS_' + str(Static.get_table_num(whois_dict['domain']))
                    # whowas
                    ...
                    set_whois_info_SQL = SQL_generate.SET_WHOIS_INFO(whois_dict)
