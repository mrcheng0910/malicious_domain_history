...
class Postal2Addr(object):
    def __init__(self):
        self.conn = MySQLdb.connect(host="10.245.146.43", user="root", passwd="platform", db="mal_domain_profile",
                                    charset='utf8')
        ...
        if (ll > 2):
            if (ll == 6):
                ...
                if (pro_ci):
                    ...
                    ##取4位解析
                    sql3 = 'select province,city from location.zip_code1 WHERE code regexp "%s.." LIMIT 1' % (
                    postal[:4])
...
def get_postal_locate(postal):
    a = Postal2Addr()
    li = a.get_pos(postal)
    return li

# print get_postal_locate('264209')[1]
